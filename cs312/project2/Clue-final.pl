...
read_number(Min,Max,N) :- read(N),(N == 'database' -> nl,print_game_state,nl,false ; check_number(Min,Max,N)).
...
ask_question_with_menu(Question,MenuList,Answer) :-
    repeat,
    write(Question),nl,
...
% TODO: check which rooms you can reach before giving suggestion?
% TODO: handle accusations and players that are eliminated
% TODO: ask what card if you showed a card
handle_a_turn :-
    TurnQ = 'What would you like to do?',get_turn_options(TurnOptions),
    ask_question_with_menu(TurnQ,TurnOptions,Option),nl,
...
handle_a_turn_helper(Option) :-
    ...
    write('You should ask this: '),suggestion(SuggestedP,SuggestedW,SuggestedR),
...
handle_a_turn_helper(Option) :-
    ...
    ask_question_with_menu(PersonQ,PersonList,P),
    WeaponQ = 'Which weapon did you suggest?',all_weapons(WeaponList),
    ask_question_with_menu(WeaponQ,WeaponList,W),
    RoomQ = 'Which room did you suggest?',all_rooms(RoomList),
    ask_question_with_menu(RoomQ,RoomList,R),
    ...
    ask_question_with_menu(PlayerQ,ShowList,ShowPlayer),
...
handle_a_turn_helper(Option) :-
    ...
    ask_question_with_menu(PlayerQ,AllPlayersButYou,Player),
    ...
    select(Player,AllPlayers,PlayerList),append(PlayerList,['Nobody'],ShowList),
    ask_question_with_menu(ShowQ,ShowList,ShowPlayer),
    get_player_number(ShowPlayer,ShowPlayerNum),
    note_silent_players(N,ShowPlayerNum,P,W,R),
...
handle_a_turn_helper(Option) :- 
    ...
    ask_question_with_menu(AccusationQ,AllPlayersButYou,Player),
    get_player_number(Player,N),player_string(N,PlayerStr),
    string_concat('Which person did ',PlayerStr,PersonQ1),string_concat(PersonQ1,' suggest?',PersonQ),
    all_people(PersonList),
    ask_question_with_menu(PersonQ,PersonList,P),
    ...
    ask_question_with_menu(GameOverQ,GameOverList,Over),
...
handle_a_turn_helper(Option) :-     
    ...
    print_game_state,nl,print_game_history.
...
ask_for_card(PlayerNum,P,W,R,Card) :-
    ...
    ask_question_with_menu(CardQ,Cards,Card).    
...
% TODO: implement better suggestion 
suggestion(P,W,R) :- 
...
print_database :- print_game_history, print_seen_cards.

print_game_history :- findall((P1,(P,W,R),P2,C),question(P1,(P,W,R),P2,C),L),print_each_turn(L).
...
% players_symbols_for_card(Playerlist,Card).
...
% list_from_range(Low,High,List) :- findall(X,between(Low,High,X),List). 

% unknown_card(X) :- (person(X);weapon(X);room(X)), not(holding(X,_)).
% all_unknown_cards(L) :- findall(X,unknown_card(X),L).
% 
% witnessed_questions(P1,(P,W,R),P2,C) :- question(P1,(P,W,R),P2,C),not(P2 == 0),var(C).
% 
% players_might_hold(Full_list) :- findall(P,witnessed_questions(P1,(P,W,R),P2,C),People), findall(W,witnessed_questions(P1,(P,W,R),P2,C),Weapons), findall(R,witnessed_questions(P1,(P,W,R),P2,C),Rooms), append(People,Weapons,Twothirds), append(Rooms,Twothirds,Full_list).

% handle_turn :- 
    % turn(N),player_number(N),!,player(N,Player),
    % write('*** Your turn! ***'),nl,
    % write('You should ask this: '),suggestion(SuggestedP,SuggestedW,SuggestedR),
    % write(SuggestedP),write(' in the '),write(SuggestedR),write(' with the '),write(SuggestedW),nl,    
    % PersonQ = 'Which person did you suggest?',all_people(PersonList),
    ...
    % ask_question_with_menu(RoomQ,RoomList,R),
    % PlayerQ = 'Who showed you a card?',all_players(AllPlayers),
    % select(Player,AllPlayers,PlayerList),append(PlayerList,['Nobody'],PlayerListAndNobody),
    % ask_question_with_menu(PlayerQ,PlayerListAndNobody,ShowPlayer),
    % get_player_number(ShowPlayer,ShowPlayerNum),
    % note_silent_players(N,ShowPlayerNum,P,W,R),
    % ask_for_card(ShowPlayerNum,P,W,R,Card),nl,
    % assert_known(Card,ShowPlayerNum),
    % write('Before question.'),
    % assert_question(N,(P,W,R),ShowPlayerNum,Card),
    % write('After question.'),
    % retract(turn(N)),next_turn(N,N1),assert(turn(N1)).
    
% handle_turn :- 
    % turn(N),not(player_number(N)),!,player(N,Player),
    % player_string(N,PlayerStr),
    % write('*** '),write(PlayerStr),write('\'s turn! ***'),nl,
    % TurnQ = 'What would you like to do?',get_their_turn_options(TurnOptions),
    % ask_question_with_menu(TurnQ,TurnOptions,T),
    % write(T),
    % (T == 1 -> handle_their_suggestion ; true),
    % retract(turn(N)),next_turn(N,N1),assert(turn(N1)),nl.

% handle_their_suggestion :-
    % turn(N),player(N,Player),player_string(N,PlayerStr),
    % string_concat('Which person did ',PlayerStr,PersonQ1),string_concat(PersonQ1,' suggest?',PersonQ),
    % all_people(PersonList),
    % ask_question_with_menu(PersonQ,PersonList,P),
    % string_concat('Which weapon did ',PlayerStr,WeaponQ1),string_concat(WeaponQ1,' suggest?',WeaponQ),
